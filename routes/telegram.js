...
router.post('/webhook', (req, res) => {
  ...
  try {
    ...
    if (text.startsWith('/start')) {
      // Welcome message sent only to THIS specific user (not broadcast to everyone)
      const { sendTelegramMessage: sendToAll, _sendToChat } = require('../telegram');
      ...
      sendTelegramMessage(`📊 Curry Tracker Bot\n✅ Active\n👥 ${ids.length} device(s) connected\n\n<i>All devices receive alerts for messages, expenses & payments.</i>`).catch(() => {});
    }
