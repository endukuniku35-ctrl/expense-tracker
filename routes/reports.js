...
router.get('/export', requireAuth, async (req, res) => {
  ...
  if (type === 'csv') {
    try {
      ...
      return res.status(500).json({ success: false, message: 'CSV export failed: ' + e.message });
    }
  }
  ...
  if (type === 'excel') {
    try {
      ...
      return res.status(500).json({ success: false, message: 'Excel export failed: ' + e.message });
    }
  }
  ...
  if (type === 'pdf') {
    try {
      ...
      return res.status(500).json({ success: false, message: 'PDF export failed: ' + e.message });
    }
  }
});
